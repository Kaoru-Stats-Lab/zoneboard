...
export function LiveMatchControls({ state, t, variant }: Props) {
  ...
  /** Broadcast: start tucked so the pitch stays clear until needed. */
  const [open, setOpen] = useState(() => variant !== "broadcast");
  ...
  return (
    <div
      ...
      }${isBroadcast && !open ? " live-match-controls--tucked" : ""}`}
      aria-label={t("scoreLabel")}
    >
      {isBroadcast && !open && (
        <button
          type="button"
          className="live-match-tuck-btn"
      ...
      {open && (
        <>
          {!pkOpOpen && (
            <>
              <div className="live-match-toolbar">
                <span className="live-match-score">
                  {home} – {away}
                </span>
                <button
              ...
              {recentLive.length > 0 && (
                <ul className="live-match-recent" aria-label={t("liveRecent")}>
                  {recentLive.map((ev) => (
                    <li
                      key={`${ev.kind}-${ev.id}`}
                      className="live-match-recent-item"
                    >
          ...
          {pkOpOpen && (
            <div className="live-match-pk-op" aria-label={t("livePkShow")}>
              <div className="live-match-pk-op-head">
                ...
                {!isBroadcast && (
                  <span className="live-match-pk-op-hint">
                    {t("pkBannerHint")}
                  </span>
                )}
                <button
