...
| 軸 | ZoneBoard | Google スライド | TACTICALista | TacticalPad 系 | 軽量 Web ボード群 | The Tactics App |
|---|---|---|---|---|---|---|
| **配信用に chrome を消す契約** | ●（≥80%） | — | △（全画面でもツール残） | △（分析・練習寄り） | —〜△ | — |
| **自分のロゴ透かし** | ● | 手作業 | △（運営寄り） | △ | — | — |
| **登録不要・ローカル** | ● | Google 前提 | クラウド前提 | アプリ課金 | まちまち | アプリ＋クラウド同期 |
| **OBS 前提の境界** | ●（顔は OBS） | 共有画面全体 | 編集机 | コーチ机 | 図だけ | コーチ机（セッション） |
| **戦術記号の厚さ** | 必要十分 | 弱い | ● | ●● | △ | ●（ドリル・器具） |
| **アニメ / 動画解析** | —（Avoid） | — | △〜● | ● | — | ●（4K 書き出し） |
| **投稿用 SNS 画角** | ●（4:5 / Story / Square） | 手切り | 画像書き出し中心 | まちまち | △ | △（動画中心） |
| **価格の入り方** | 今は無料コア | 無料＋アカウント | Freemium | 年額アプリ | 無料〜広告 | 月額 £8.99〜 |
...
| **The Tactics App** | コーチ机のアニメ・4K書き出し。ライブ OBS 席ではない（§0i） |
...
### 0i. The Tactics App — コーチ机のアニメ席（公開サイト · 2026-08-30）

**現場:** [thetacticsapp.com](https://thetacticsapp.com/) · ベータ登録 · プランは Coming Soon。  
**判定:** **tactical-board / TACTICALista と同じ席。** 練習前の教材・分析・スカウト。ZB v1 の OBS ライブ席とは交差しない。**ユーザーへの冷 DM はしない。**

| The Tactics App | ZoneBoard v1 | 方針 |
|---|---|---|
| **インストール型アプリ** · オフライン可 · 復帰後クラウド同期 | ブラウザ · 登録不要 · localStorage | 楔は逆（アカウント不要） |
| **アニメ** · **4K 動画書き出し（秒）** | PNG · 局面切替 · 手動ドラッグ | 教材 MP4 は持たない（§0c） |
| ドリル · 器具 · セッション設計 | 駒・Pass/Run/Pen/Zone | 練習図は v1 非目標 |
| スカウトメモ · 名簿保存 | 名簿ペースト · 選手 DB なし | 分析デスクではない |
| コンテンツクリエイターも FAQ に含む | 配信者（OBS 窓）が先 | 「クリエイター」でもジョブが違う |
| Basic **£8.99/月** · Pro **£13.99** · Club カスタム | 今は無料コア | 価格で殴らない |

**LP / X:** 社名を出さない。差は「4K より速い」ではなく **今夜の配信に窓を足す / B で消える / 芝に運営ロゴなし**。
