...
色展開（チームカラーのボール差し替え）は **やらない**。競技ごとに **世界で通じる1種**を基本とする。

**例外（サッカーのみ · 2026-09-06）:** CL 解説の夜向けに **スターボール・ピクトグラム**を追加可（`soccer_starball.svg` · `board.soccerBallLook`）。公式シーズン球のカタログ（28種）・Adidas/UEFA ロゴのトレースは **禁止**。見た目はフラット・高コントラストのまま。
...
- **CL 公式試合球の年鑑コピー**（TNT 等のコラージュ切り出し・シーズン28種）
