...
**更新:** 2026-09-01（**es · pt · pl · de · fr · tr · it 出荷済** · 第3波 it 完了）  
**きっかけ:** PH 公開面を英語一本にしたあと、「5大リーグ分（英西伊独仏）の UI 翻訳が要るか」→ 天皇杯同時視聴の壁打ち用に **ボード chrome だけ JA** を開放。  
**結論:** 市場は広い。**第1波** es→pt→pl · **第2波** de→fr→tr · **第3波** **it 出荷済**（2026-09-01）。公開デフォルト EN。**3バック可変 preset**（B 層）は it 向け Must · **未実装 · Later**。
...
| 面 | 言語 |
|----|------|
| LP・法務・Guide・静的ページ | **英語** + **`/es/` … `/it/`** 各ロケール LP |
| ボード chrome | Settings · `?lang=` · `/ja/board` … `/it/board` |
| 永続化 | `locale: "en" \| "ja" \| "es" \| "pt" \| "pl" \| "de" \| "fr" \| "tr" \| "it"` |
...
| `/es/` · … · `/it/` | 各ロケール LP（`dist/<locale>/index.html`） |
| `/` · `/ja/` · … · `/it/` | 各ロケール LP（`dist/<locale>/index.html`） |
| `/ja` | `/ja/` へ 301（**日本語 LP 出荷済**） |
...
**位置づけ:** Fit **74%**。**2026-09-01 `/it/` 出荷済**。3 バック可変 preset は B 層 · Later。
...
**結論:** 西欧中欧東欧調査の **シナジー TOP4 は UI 面で充足**（**it 出荷済 2026-09-01**）。次のレバーは **B 層**（3 バック可変 · キット/フォーメ）— C（chrome）だけ増やさない。
...
2. **同じ LP URL に第二言語を積まない。** `/es/` … **`/it/`**。**第3波 it 出荷済**（2026-09-01）。
3. **UI ローカライズ:** 第1波 **済** · 第2波 **済** · **第3波 it 済**。
4. **日本語**は `/ja/` LP + ボード chrome。PH グローバル面は `/`（英語）を維持。
...
**第3波 it 出荷済**（2026-09-01）。**3 バック可変 preset**（B 層）は Later。

既存メモ: B-074–078（**pl/de/fr/tr/it 済**）。2026-09-01: **9 ロケール chrome**（ja + 8 LP 面）。
