...
**更新:** 2026-08-24  
**きっかけ:** PH 公開面を英語一本にしたあと、「5大リーグ分（英西伊独仏）の UI 翻訳が要るか」  
**結論:** 市場は広い。ただし **リーグ ≠ 言語**。今は英語だけ。次の UI は流入を見て **es → pt-BR → fr**。独・伊は5大揃えのためには足さない。

関連: [`PRODUCT_NOTE.md`](PRODUCT_NOTE.md) §4-3 · [`SPEC.md`](SPEC.md) §9 · [`BACKLOG.md`](BACKLOG.md) B-035–037 · [`FUTSAL_BEACH_RESEARCH.md`](FUTSAL_BEACH_RESEARCH.md)
...
公開面に FR/DE/ES/IT を同じ URL へ積むのは、日本語を積まない理由と同じでやらない。
...
| 面 | 言語 |
|----|------|
| LP・法務・Guide・ボード chrome | **英語のみ**（`src/i18n/locale.ts` の `APP_LOCALE = "en"`） |
| `messages.ja` / `src/i18n/howTo.ts` の JA / `src/site/pages.ts` の Ja フィールド | ソースに残す。あとで `/ja/` |
| ピッチ上の駒名・クラブ名 | ユーザ入力。フォントはラテン拡張・キリル・日本語を守る（[`STUDIO.md`](STUDIO.md)） |
| アラビア語・ヘブライ語（RTL） | Later（同） |

PH と欧州戦術・OBS まわりの共通語は英語。同じページに第二言語を載せない。
...
2. **同じ URL に第二言語を積まない。** 足すときは `/es/` `/pt/` `/fr/` `/ja/` のように面を分ける。
3. **UI の次点はリーグ順ではない。** 既存バックログと同じく **es → pt-BR**。その次の候補は **fr**（アフリカ L2 を含む）。独・伊は流入が出てから。
4. **日本語**は作者言語としてソースに残す。PH 面には出さない。JP ローンチは `/ja/`。
5. **5大リーグ向けに先にやるなら C ではなく B。** キットとフォーメ。翻訳の代用にしない。
